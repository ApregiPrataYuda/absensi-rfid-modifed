<script>
    ...
    const STAFF_LABEL = STAFF_CONTEXT === 'piket' ? 'piket' : 'Mandor';
    const STAFF_LABEL_TITLE = STAFF_CONTEXT === 'piket' ? 'Piket' : 'Mandor';
    const STAFF_KELAS_LABEL = STAFF_CONTEXT === 'piket' ? 'Petugas Gedung - Lantai' : 'Pengelola Gedung - Lantai';
    const STAFF_HINT_TEXT = STAFF_CONTEXT === 'piket'
        ? 'Jika dipilih, petugas piket bisa difokuskan pada gedung tertentu.'
        : 'Jika dipilih, foreman(mandor) hanya bisa melihat karyawan(tukang) di gedung yang dia kelola.';
    const STAFF_METHODS = {
        ...
        <div class="bg-white rounded-2xl shadow-2xl overflow-hidden max-w-3xl w-full">
            ...
            <div class="p-6 max-h-[75vh] overflow-y-auto">
                <form onsubmit="saveGuru(event, ${isEdit})" class="space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            <label class="${labelClass}">Username Foreman (${STAFF_LABEL_TITLE})</label>
                            <input name="username" value="${guru?.username || ''}" placeholder="Username" required class="${inputClass}">
                        </div>
                        <div>
                            <label class="${labelClass}">Nama Foreman (${STAFF_LABEL_TITLE})</label>
                            <input name="nama" value="${guru?.name || ''}" placeholder="Nama lengkap ${STAFF_LABEL}" required class="${inputClass}">
                        </div>
                        <div>
                            <label class="${labelClass}">Email Foreman (${STAFF_LABEL_TITLE})</label>
                            <input type="email" name="email" value="${guru?.email || ''}" placeholder="email@foremanExample.co.id (opsional)" class="${inputClass}">
                        </div>
                        <div>
                            <label class="${labelClass}">Password Foreman (${STAFF_LABEL_TITLE})</label>
                        ...
                        <div>
                            <label class="${labelClass}">No HP Foreman (${STAFF_LABEL_TITLE})</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                        <div>
                            <label class="${labelClass}">Jenis Kelamin Foreman (${STAFF_LABEL_TITLE})</label>
                            <select name="jenisKelamin" class="${inputClass}">
                        ...
                        <div>
                            <label class="${labelClass}">Tanggal Lahir Foreman (${STAFF_LABEL_TITLE})</label>
                            <input type="date" name="tanggalLahir" value="${tanggalLahir}" class="${inputClass}">
                        </div>
                        <div>
                            <label class="${labelClass}">Agama Foreman (${STAFF_LABEL_TITLE})</label>
                            <select name="agama" class="${inputClass}">
                    ...
                    <div>
                        <label class="${labelClass}">Alamat Foreman (${STAFF_LABEL_TITLE})</label>
            ...
            <div class="p-6 max-h-[70vh] overflow-y-auto space-y-6">
                <div>
                    <h4 class="text-sm font-bold text-violet-700 mb-3 flex items-center gap-2">
                        <i class="fas fa-id-badge"></i> Data Detail Akun
                    </h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
...
function handleArchiveAndReset() {
    ...
    function patchedPopulateFilterKelas(id, rows) {
        ...
        select.innerHTML = '<option value="">Semua Gedung</option>' + kelasList
    ...
    window.renderGuruRows = function renderGuruRowsPatched(data, startIdx) {
        ...
        if (!data || data.length === 0) {
            tbody.innerHTML = `<tr><td colspan="7" class="p-8 text-center text-gray-400">Data ${STAFF_LABEL} tidak ditemukan.</td></tr>`;
            return;
        }

    ...
    window.loadDashboardData = function loadDashboardDataPatched() {
        ...
        patchedRun('getMonitoringRealtime', kelasFilter)
            .then((res) => {
                ...
                const alpa = rows.filter((r) => r.status === 'Alpa' || r.status === 'Alfa').length;
                const belum = rows.filter((r) => r.status === 'Belum Absen').length;
                const libur = rows.filter((r) => r.status === 'Libur').length;
                const total = rows.length;
                const totalAktif = Math.max(total - libur, 0);
                ...
                if (document.getElementById('view-siswa-dashboard')) {
                    ...
                    const siswaRow = rows.find((r) => String(r.nisn) === nisn) || rows.find((r) => String(r.nama) === displayName);
                    ...
                    if (totalAktif <= 0) {
                        ...
                        patchedSetText('studentAttendanceSummary', `${hadir} dari ${totalAktif} karyawan hadir di kantor hari ini.`);
                    }
